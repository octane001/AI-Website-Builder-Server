...
// Controller Fuction to Make Revision
...
export const rollBackToVersion = async (req: Request, res: Response) => {
    try {
        ...
        if (!version) {
            return res.status(404).json({ message: "Version not fond" })
        }
        await prisma.websiteProject.update({
            where: { id: projectId, userId },
            data: {
...
export const getPublishedProjects = async (req: Request, res: Response) => {
    try {
        const publishedProjects = await prisma.websiteProject.findMany({
...
// Controller to save projects code
